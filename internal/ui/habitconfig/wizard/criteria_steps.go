...
// AIDEV-NOTE: Criteria step handler pattern for elastic habit mini/midi/maxi steps
// This handler demonstrates:
// - Conditional step skipping (shouldSkip method)
// - Level-specific configuration (level field for "mini"/"midi"/"maxi")
// - Dynamic form field generation based on habit type
// Reuse this pattern for elastic habit criteria steps with different levels
...
func (h *CriteriaStepHandler) initializeForm(state State) {
	...
	if h.habitType == models.SimpleHabit {
		...
		// Get field type - for now default to unsigned int for elastic habits
