...
// AIDEV-NOTE: testing-patterns; uses NewChecklistHabitCollectionFlow() and CollectEntryDirectly() for headless testing
// All major scenarios covered: checklist completion, automatic scoring with criteria, manual scoring, error handling

func TestChecklistHabitCollectionFlow(t *testing.T) {
...
checklists:
  - id: "test_checklist"
	...
	flow := NewChecklistHabitCollectionFlow(factory, scoringEngine, checklistsPath)

	// Test flow type identification
	if flow.GetFlowType() != string(models.ChecklistHabit) {
		t.Errorf("GetFlowType() = %v, want %v", flow.GetFlowType(), string(models.ChecklistHabit))
	}

	// Test scoring requirement
	if !flow.RequiresScoring() {
		t.Errorf("RequiresScoring() expected true for checklist habits")
	}
...
func TestChecklistHabitLoadChecklistData(t *testing.T) {
	// Create temporary test directory
	tempDir := t.TempDir()
	checklistsPath := filepath.Join(tempDir, "checklists.yml")
...
checklists:
  - id: "test_checklist"
	...
	checklist, err := flow.loadChecklistData(habit)
	if err != nil {
...
func TestChecklistHabitLoadChecklistDataErrors(t *testing.T) {
	factory := NewEntryFieldInputFactory()
	scoringEngine := &scoring.Engine{}

	// Test with non-existent file
	flow := NewChecklistHabitCollectionFlow(factory, scoringEngine, "/nonexistent/path.yml")
	...
	_, err := flow.loadChecklistData(habit)
	if err == nil {
...
func TestChecklistHabitCollectEntryDirectly(t *testing.T) {
	// Create temporary test directory
	tempDir := t.TempDir()
	checklistsPath := filepath.Join(tempDir, "checklists.yml")
  ...
  - id: "test_checklist"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := flow.CollectEntryDirectly(habit, tt.value, "test notes", nil)
...
func TestChecklistHabitDetermineTestingAchievementLevel(t *testing.T) {
	// Create temporary test directory
	tempDir := t.TempDir()
	checklistsPath := filepath.Join(tempDir, "checklists.yml")
...
checklists:
  - id: "test_checklist"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			level := flow.determineTestingAchievementLevel(habit, tt.value)
...
func TestChecklistHabitDetermineTestingAchievementLevelErrors(t *testing.T) {
	factory := NewEntryFieldInputFactory()
	scoringEngine := &scoring.Engine{}
	...
	level := flow.determineTestingAchievementLevel(habit, []string{"Item 1"})
...
func TestChecklistHabitAutomaticScoringWithCriteria(t *testing.T) {
	// Create temporary test directory
	tempDir := t.TempDir()
	checklistsPath := filepath.Join(tempDir, "checklists.yml")
