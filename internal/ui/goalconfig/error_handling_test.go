...
func TestFilePermissionHandling(t *testing.T) {
	...
	t.Run("read-only file handling", func(t *testing.T) {
		goalsFile := filepath.Join(tempDir, "readonly_goals.yml")
...
`,
		...
		{
			name: "missing goals array",
			yamlContent: `
version: "1.0.0"
created_date: "2024-01-01"
`,
			expectError: false, // Should default to empty goals array
		},
		{
			name: "goals is not an array",
			yamlContent: `
version: "1.0.0"
created_date: "2024-01-01"
goals: "not an array"
...
func TestGoalValidationEdgeCases(t *testing.T) {
	t.Run("goal with extremely long title", func(t *testing.T) {
		...
		goal := &models.Goal{
			...
			Description: "Goal with very long title",
	...
	t.Run("goal with special characters in title", func(t *testing.T) {
		specialTitles := []string{
			"Goal with émojis 🎯",
			"Goal with 中文 characters",
			"Goal with symbols !@#$%^&*()",
		...
		for _, title := range specialTitles {
			t.Run(title, func(t *testing.T) {
				goal := &models.Goal{
					Title:       title,
					Description: "Goal with special characters",
					GoalType:    models.SimpleGoal,
					FieldType: models.FieldType{
						Type: models.BooleanFieldType,
					},
	...
	t.Run("goal with empty description", func(t *testing.T) {
		goal := &models.Goal{
	...
	t.Run("goal with whitespace-only title", func(t *testing.T) {
		goal := &models.Goal{
			Title:       "   \t\n   ",
			Description: "Goal with whitespace title",
			GoalType:    models.SimpleGoal,
			FieldType: models.FieldType{
				Type: models.BooleanFieldType,
			},
...
func TestConcurrentAccessSafety(t *testing.T) {
	tempDir := t.TempDir()
	goalsFile := filepath.Join(tempDir, "concurrent_goals.yml")
	entriesFile := filepath.Join(tempDir, "entries.yml")
...
func TestMemoryAndResourceUsage(t *testing.T) {
	t.Run("large number of goals", func(t *testing.T) {
		// Create schema with many goals
		schema := &models.Schema{
			Version:     "1.0.0",
			CreatedDate: "2024-01-01",
			Goals:       make([]models.Goal, 1000),
		}

		// Fill with valid goals
		for i := 0; i < 1000; i++ {
			schema.Goals[i] = models.Goal{
				Title:       fmt.Sprintf("Goal %d", i),
				Description: fmt.Sprintf("Test goal number %d", i),
				GoalType:    models.SimpleGoal,
				FieldType: models.FieldType{
					Type: models.BooleanFieldType,
				},
	...
	t.Run("deeply nested structures", func(t *testing.T) {
		// Create goal with complex criteria
		complexGoal := &models.Goal{
			Title:       "Complex Goal",
			Description: "Goal with complex criteria structure",
