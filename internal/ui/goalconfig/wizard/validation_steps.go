...
// AIDEV-NOTE: Validation step handler for elastic goals - validates mini ≤ midi ≤ maxi constraints
...
// - Preview of complete goal configuration before final confirmation

// ValidationStepHandler handles validation of elastic goal criteria constraints
...
func NewValidationStepHandler(goalType models.GoalType) *ValidationStepHandler {
...
func (h *ValidationStepHandler) CanNavigateTo(state State) bool {
	...
	if h.goalType != models.ElasticGoal {
		return true // Skip validation for non-elastic goals
	}
...
func (h *ValidationStepHandler) GetDescription() string {
	return "Validate goal configuration and criteria constraints"
}
		...
		fields = append(fields,
			huh.NewSelect[string]().
				...
				Options(
					...
					huh.NewOption("Cancel goal creation", "cancel"),
				).
				Value(&h.action),
		)
	}
...
func (h *ValidationStepHandler) runValidation(state State) {
	...
	if h.goalType != models.ElasticGoal {
		return // No validation needed for non-elastic goals
	}
