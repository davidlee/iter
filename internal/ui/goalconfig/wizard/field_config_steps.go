...
// AIDEV-NOTE: Field configuration step handler for elastic and informational goals
...
// FieldConfigStepHandler handles field type configuration for elastic and informational goals
...
func NewFieldConfigStepHandler(goalType models.GoalType) *FieldConfigStepHandler {
	...
	if h.formComplete {
		if stepData := state.GetStep(h.getStepIndex()); stepData != nil {
			if data, ok := stepData.(*FieldConfigStepData); ok {
				...
				if h.goalType == models.InformationalGoal && h.direction != "" {
...
func (h *FieldConfigStepHandler) GetDescription() string {
	return "Configure the data type and constraints for your goal"
}
...
func (h *FieldConfigStepHandler) initializeForm(state State) {
	...
	fields = append(fields,
		huh.NewSelect[string]().
			Title("Field Type").
			Description("What type of data will you track for this goal?").
	...
	if h.goalType == models.InformationalGoal {
		...
		// Direction for informational goals
		fields = append(fields,
			huh.NewSelect[string]().
				Title("Direction").
