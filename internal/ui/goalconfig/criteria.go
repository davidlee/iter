...
// CreateSimpleCriteriaForm creates a form for simple goal criteria
		...
		form := huh.NewForm(
			huh.NewGroup(
				...
				huh.NewInput().
					...
					Description("Explain what achieving this goal means").
...
// CreateElasticCriteriaForm creates forms for elastic goal criteria (mini/midi/maxi)
func (cb *CriteriaBuilder) CreateElasticCriteriaForm(fieldType models.FieldType, level string) (*huh.Form, *CriteriaConfig) {
	config := &CriteriaConfig{}

	if fieldType.Type == models.BooleanFieldType {
		// Boolean elastic goals don't make much sense, but handle gracefully
		form := huh.NewForm(
			huh.NewGroup(
				huh.NewInput().
