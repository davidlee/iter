...
// ElasticGoalCreator implements a bubbletea model for creating elastic goals
type ElasticGoalCreator struct {
	form     *huh.Form
	quitting bool
	err      error
	result   *models.Goal
	...
	// Field configuration data - reuses SimpleGoalCreator patterns
...
// NewElasticGoalCreator creates a new elastic goal creator with pre-populated basic info
func NewElasticGoalCreator(title, description string, goalType models.GoalType) *ElasticGoalCreator {
...
func (m *ElasticGoalCreator) CreateGoalDirectly() (*models.Goal, error) {
	return m.createGoalFromData()
}

// Init implements tea.Model - called when the model is first initialized
func (m *ElasticGoalCreator) Init() tea.Cmd {
...
func (m *ElasticGoalCreator) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
...
func (m *ElasticGoalCreator) View() string {
	if m.quitting {
		if m.err != nil {
			return fmt.Sprintf("Error creating elastic goal: %v\n", m.err)
		}
		if m.result != nil {
			return fmt.Sprintf("✅ Elastic goal created successfully: %s\n", m.result.Title)
		}
		return "Elastic goal creation cancelled.\n"
	}
...
func (m *ElasticGoalCreator) GetResult() (*models.Goal, error) {
	if m.err != nil {
		return nil, m.err
	}
	return m.result, nil
...
func (m *ElasticGoalCreator) IsCompleted() bool {
	return m.result != nil && m.err == nil
}

// IsCancelled returns true if the form was cancelled
func (m *ElasticGoalCreator) IsCancelled() bool {
...
func (m *ElasticGoalCreator) initializeStep() {
...
func (m *ElasticGoalCreator) adjustFlowForFieldType() {
...
func (m *ElasticGoalCreator) adjustFlowForScoringType() {
...
func (m *ElasticGoalCreator) isCurrentStepScoringType() bool {
...
func (m *ElasticGoalCreator) needsFieldConfiguration() bool {
...
func (m *ElasticGoalCreator) createFieldTypeSelectionForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewSelect[string]().
				Title("Field Type").
				Description("Choose what type of data this elastic goal will collect\n(Elastic goals track achievement levels: mini/midi/maxi)").
				Options(
...
// createFieldConfigurationForm creates the field configuration form (reuses SimpleGoalCreator patterns)
func (m *ElasticGoalCreator) createFieldConfigurationForm() *huh.Form {
	switch m.selectedFieldType {
	case "numeric":
...
// createNumericConfigForm creates numeric field configuration form (reused from SimpleGoalCreator)
func (m *ElasticGoalCreator) createNumericConfigForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewSelect[string]().
				Title("Numeric Type").
...
// createTextConfigForm creates text field configuration form (reused from SimpleGoalCreator)
func (m *ElasticGoalCreator) createTextConfigForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewConfirm().
...
func (m *ElasticGoalCreator) createScoringTypeForm() *huh.Form {
	...
	return huh.NewForm(
		huh.NewGroup(
			huh.NewSelect[models.ScoringType]().
				Title("Scoring Type").
				Description("How should elastic goal achievement levels be determined?").
				Options(options...).
				Value(&m.scoringType),
		),
	)
...
func (m *ElasticGoalCreator) supportsAutomaticScoring() bool {
	switch m.selectedFieldType {
	case models.TextFieldType:
...
func (m *ElasticGoalCreator) createCriteriaDefinitionForm() *huh.Form {
	// For elastic goals, we need to collect criteria for all three tiers
	// This is more complex than simple goals and requires careful UX design
	switch m.selectedFieldType {
...
func (m *ElasticGoalCreator) createNumericElasticCriteriaForm() *huh.Form {
	unit := m.unit
	if unit == "" {
		unit = "units"
	}
		...
		huh.NewGroup(
			huh.NewInput().
				...
				Validate(func(s string) error {
					...
					// AIDEV-NOTE: real-time-validation; prevents invalid orderings during form entry (not just at goal creation)
...
func (m *ElasticGoalCreator) createTimeElasticCriteriaForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewNote().
...
func (m *ElasticGoalCreator) createDurationElasticCriteriaForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewNote().
...
func (m *ElasticGoalCreator) validateTimeInput(s string) error {
	if strings.TrimSpace(s) == "" {
		return fmt.Errorf("time value is required")
	}
	// Basic time format validation (same as SimpleGoalCreator)
...
func (m *ElasticGoalCreator) validateDurationInput(s string) error {
	if strings.TrimSpace(s) == "" {
...
// createPromptAndCommentForm creates the final prompt and comment form (reused from SimpleGoalCreator)
func (m *ElasticGoalCreator) createPromptAndCommentForm() *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			huh.NewInput().
				Title("Goal Prompt").
				Description("The question asked when tracking this elastic goal").
				Value(&m.prompt).
				Validate(func(s string) error {
					if strings.TrimSpace(s) == "" {
...
// AIDEV-NOTE: elastic-goal-builder; constructs goal with mini/midi/maxi criteria validation
// createGoalFromData creates a models.Goal from the collected form data
func (m *ElasticGoalCreator) createGoalFromData() (*models.Goal, error) {
	// Build field type configuration (reuses SimpleGoalCreator patterns)
...
func (m *ElasticGoalCreator) buildCriteriaFromData(tier string) (*models.Criteria, error) {
	...
	switch m.selectedFieldType {
	case "numeric", models.UnsignedIntFieldType, models.UnsignedDecimalFieldType, models.DecimalFieldType:
		// Numeric criteria - all tiers use greater_than_or_equal for elastic goals
		unit := m.unit
		if unit == "" {
			unit = "units"
		}
...
func (m *ElasticGoalCreator) getResolvedFieldType() string {
